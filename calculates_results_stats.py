...
def calculates_results_stats(results_dic):
    ...
    results_stats_dic = {
        'n_images': 0, 'n_dogs_img': 0, 'n_notdogs_img': 0,
        'n_match': 0, 'n_correct_dogs': 0, 'n_correct_notdogs': 0, 'n_correct_breed': 0,
        'pct_match': 0.0, 'pct_correct_dogs': 0.0, 'pct_correct_notdogs': 0.0, 'pct_correct_breed': 0.0
    }
    ...
    for key in results_dic:
        ...
        # Count of not-dog images
        else:
            results_stats_dic['n_notdogs_img'] += 1

            # Count of correctly classified not-dog images
            if results_dic[key][4] == 0:
                results_stats_dic['n_correct_notdogs'] += 1

        # Count of label matches
        if results_dic[key][2] == 1:
