...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    """
    # Read dog names from file and store them in a dictionary
    dognames_dic = {}

    with open(dogfile, "r") as infile:
        for line in infile:
            dog_name = line.strip().lower()  # Ensure case consistency
            if dog_name not in dognames_dic:
                dognames_dic[dog_name] = 1
            else:
                print(f"Warning: Duplicate dog name found: {dog_name}")

    # Process each result in results_dic
    for key in results_dic:
        # Check if pet image label is a dog
        is_pet_dog = 1 if results_dic[key][0] in dognames_dic else 0

        # Check if classifier label is a dog (can include multiple terms separated by commas)
        is_classifier_dog = 0
        for label in results_dic[key][1].split(", "):  # Split classifier label by commas
            if label in dognames_dic:
                is_classifier_dog = 1
                break

        # Extend the result list with the dog classification results
